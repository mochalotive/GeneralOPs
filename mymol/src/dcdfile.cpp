...
void DCDFile::skipFrame()
{
  ...
  read(reinterpret_cast<char *>(&dummyInt), sizeof(int));
...
DCDFile &operator>>(DCDFile &dcdFile, Geometry &geometry)
{
  ...
  int length;
  dcdFile.read(reinterpret_cast<char *>(&length), sizeof(int));
  if(length != 48) // Length of unit cell field should be 48
  {
    std::cerr << "Error reading .dcd file " << dcdFile.fileName() << ": Invalid file format - read unit cell data Geometry 1." << std::endl;
    dcdFile.clear();
    return dcdFile;
  }
  ...
  if(length != 48) // Length of unit cell field should be 48
  {
    std::cerr << "Error reading .dcd file " << dcdFile.fileName() << ": Invalid file format - read unit cell data Geometry 2." << std::endl;
    dcdFile.clear();
    return dcdFile;
  }
  // Skip four times the number of atoms
  for(int i = 0; i < 4; ++i) dcdFile.get();
  // Read x-coordinates
  for(int i = 0; i < dcdFile.numAtoms_; ++i)
  {
    SmallReal newx;
...
DCDFile &operator>>(DCDFile &dcdFile, System<Geometry> &system)
{
  ...
  if(length != 48) // Length of unit cell field should be 48
  {
    std::cerr << "Error reading .dcd file " << dcdFile.fileName() << ": Invalid file format - read unit cell data System<Geometry> 1." << std::endl;
    dcdFile.clear();
    return dcdFile;
  }
  // Read unit cell information
  ...
  if(length != 48) // Length of unit cell field should be 48
  {
    std::cerr << "Error reading .dcd file " << dcdFile.fileName() << ": Invalid file format - read unit cell data System<Geometry> 2." << std::endl;
    dcdFile.clear();
    return dcdFile;
  }
  // Skip four times the number of atoms
  ...
  // Read x-coordinates
  for(int i = 0; i < system.size(); ++i)
  for(int j = 0; j < system[i].numAtoms(); ++j)
  {
    SmallReal newx;
    dcdFile.read(reinterpret_cast<char *>(&newx), sizeof(SmallReal));
...
DCDFile &operator>>(DCDFile &dcdFile, System<Molecule> &system)
{
  ...
  if(length != 48) // Length of unit cell field should be 48
  {
    std::cerr << "Error reading .dcd file " << dcdFile.fileName() << ": Invalid file format System<Molecule> 1." << std::endl;
    dcdFile.clear();
    return dcdFile;
  }
  // Read unit cell information
  ...
  if(length != 48) // Length of unit cell field should be 48
  {
    std::cerr << "Error reading .dcd file " << dcdFile.fileName() << ": Invalid file format System<Molecule> 2." << std::endl;
    dcdFile.clear();
    return dcdFile;
  }
  // Skip four times the number of atoms
...
void DCDFile::initializeDCDFile()
{
  ...
  {
    std::cerr << "Error reading .dcd file " << fileName() << ": Invalid file format - DCD header 1." << std::endl;
    clear();
    return;
  }
    std::cerr <<"first header: " <<  length << std::endl;
  ...
  {
    std::cerr << "Error reading .dcd file " << fileName() << ": Invalid file format - CORD field 1." << std::endl;
    clear();
    return;
  }
  // Read the number of frames
  read(reinterpret_cast<char *>(&numFrames_), sizeof(int));
  std::cerr <<"num frames: " <<  numFrames_ << std::endl;

  // Skip the rest of the header
  for(int i = 0; i < 76; ++i) get();
  // Read another 84
  read(reinterpret_cast<char *>(&length), sizeof(int));
  std::cerr <<"second header: " <<  length << std::endl;

  if(length != 84) // Length of dcd header should be 84
  {
    std::cerr << "Error reading .dcd file " << fileName() << ": Invalid file format - DCD header 2." << std::endl;
    clear();
    return;
  }
  ...
  {
    std::cerr << "Error reading .dcd file " << fileName() << ": Invalid file format - Title record 1." << std::endl;
    clear();
    return;
  }
  ...
  {
    std::cerr << "Error reading .dcd file " << fileName() << ": Invalid file format - number of atoms field." << std::endl;
    clear();
    return;
  }
  // Read the number of atoms
  read(reinterpret_cast<char *>(&numAtoms_), sizeof(int));
  // Read another 4
  read(reinterpret_cast<char *>(&length), sizeof(int));
  if(length != 4) // Should be 4
  {
    std::cerr << "Error reading .dcd file " << fileName() << ": Invalid file format - number of atoms + 4." << std::endl;
    clear();
    return;
  }
  // Done
