...
ForceFile::ForceFile():IOFile(){};
ForceFile::ForceFile(std::string const &fileName):IOFile(fileName, OUT){};
//operator
#define R_CUTOFF 0.01
ForceFile &operator<<(ForceFile &plumedFile, PlumedParameters &parameters) 
{
...
//                  " GROUPB=" <<  parameters.myPlumedSystemParameters.residueNames[0] << " SWITCH={RATIONAL R_0=0.1 D_0=";
...
//          parameters.myPlumedSystemParameters.residueNames[i] << " SWITCH={EXP R_0=0.01 D_0=0.5 D_MAX=3.0}\n";
//  }
...
//just need to restrain one at a time

  plumedFile << "force: RESTRAINT ARG=";
  plumedFile << "sa" << parameters.myPlumedSystemParameters.residueNames[parameters.resid] << parameters.op << " "; 
  //now print OPs
  plumedFile << "AT=";
  switch (parameters.fileType[parameters.resid])
  {
  case (DOPS): {plumedFile <<parameters.mySystemDataPackage.myCrystalOrderParameters[parameters.resid].distance[parameters.op]; break;}
  case (BOPS): {plumedFile <<parameters.mySystemDataPackage.myCrystalOrderParameters[parameters.resid].bondOrientation[parameters.op]; break;}
  case (ROPS): {plumedFile <<parameters.mySystemDataPackage.myCrystalOrderParameters[parameters.resid].relativeOrientation[parameters.op]; break;}
  case (TOPS): {plumedFile <<parameters.mySystemDataPackage.myCrystalOrderParameters[parameters.resid].total[parameters.op]; break;}
  default: {break;}
  }
 
  plumedFile << " KAPPA=" << parameters.myPlumedSystemParameters.kappa[parameters.resid] << "\n"; 
return plumedFile; 
}
