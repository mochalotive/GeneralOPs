    ...
    def __add__(self, other):
        ...
        for i, tag in enumerate(self.tags):
            ...
            if tag == "NCELLS":
                ...
                continue
            if tag == "TOTAL_OPS":
                self.data[tag] = [] #for compatibility 
                continue
...
assert len(sys.argv) > 1,  "Need to supply at least one COP file!"

cop_files = sys.argv[1:len(sys.argv)]
cops : CopFile  = []
for name in cop_files:
    cops.append(CopFile.open(name))
for i, cop in enumerate(cops):
    if i==0:
        continue
    cops[0] += cop
with open("combined.cop", "w") as w:
    w.write(cops[0].to_string())
