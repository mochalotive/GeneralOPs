...
int main(int argc, char* argv[])
{
  ...
  if((found_frcweight && found_pmfweight) || (found_frcweight && found_extweight) ||
     (found_pmfweight && found_extweight))
  {
    std::cerr << "Error: The -frcweight, -pmfweight and -extweight options are mutually exclusive" << std::endl
              << "Use smcvstep -help for help." << std::endl;
    return -1;
  }
...
*/
  ...
  for(size_t i = 0; i <  numInputImgs; ++i)
  {
    ...
    // TEST
    std::cout << std::setprecision(20) << imageForceData[i].mMatrix << std::endl;
    // END TEST
  }
  ...
  else // No external file given, make equispaced
  ...
  */
  newImageOPs = imageOPsAfterEuler;
