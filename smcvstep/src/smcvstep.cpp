...
//JAKE - FIXING (as in, making them not move, not fix as in repair) THINGS JUNE 2024
  ...
  for(size_t i = 0; i < numInputImgs; ++i)
  {
    ...
    std::cout << numParameters << std::endl;
    std::cout << currentOPs.size() << std::endl;
  ...
  if(found_zerotol && zeroTolerance > 0.0)  // Apply tolerance rules
    for(size_t i = 0; i < numInputImgs; ++i)
      for(size_t j = 0; j < numParameters; ++j)
       {
        std::cout << imageForceData[i].restraintForces[j] << std::endl;
...
//        if (imageOPsAfterEuler[i][j] < 0.0)
//        {
//        imageOPsAfterEuler[i][j]=0.001;
//        }
//        else if (imageOPsAfterEuler[i][j] > 1.0)
//        {
//        imageOPsAfterEuler[i][j] = 0.999;
//        }
         }
      }
    }
